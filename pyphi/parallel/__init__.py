...
def set_flatten(items, branch=False):
    if branch:
        item_set = set().union(*items)
        return item_set

    itemset = set()
    itemset.union(*items)
    print("returning " + str(type(itemset)) + " of size " + str(len(item_set)))

    # elt = itemset.pop()
    # print("itemset return type: " + str(type(elt)))
    # itemset.add(elt)
    return itemset
...
def _reduce(results, reduce_func, reduce_kwargs, branch):
    if reduce_func is _flatten or reduce_func is set_flatten:
